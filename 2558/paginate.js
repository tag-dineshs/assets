( function( $ ) {
	$.fn.paginate = function( options ) {
		var defaults = {
			pagination_class: "custom-ticket-list",
			items_per_page: 5,
			prev_next: true,
			prev_text: '&laquo;',
			next_text: '&raquo;'
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="prev"><a href="javascript:void(0)">' + options.prev_text + '</a></li>' );
			}
			...
			// Add Next Button
			if ( options.prev_next == true ) {
				obj.find( "." + options.pagination_class ).append( '<li class="next"><a href="javascript:void(0)">' + options.next_text + '</a></li>' );
			}
		}
		...
		function showPage( page_number ) {
			...
			obj.children().not( '.custom-ticket-list__controls' ).css( 'display', 'none' ).slice( start_from, end_on ).css( 'display', 'block' );
		}

		createPaginationControls( pages );
		showPage( 0 );

		obj.find( '.custom-ticket-list li' ).not( ".prev,.next" ).first().addClass( 'active' );

		// Navigate to the coresponding slide when clicking on a nav-control
		obj.find( '.custom-ticket-list li' ).not( ".prev,.next" ).click( function() {
		...
		obj.find( '.custom-ticket-list li.prev' ).click( function() {
		...
		obj.find( '.custom-ticket-list li.next' ).click( function() {
