	...
	*/
	...
	return this.each(function()
	{
		var container_id = '#' + $(this).attr('id'),
			items = $(this).find(settings.pagination_container),
		...
		function refresh_previous(page_number)
		{
			...
			previous_html += '<span class="';
			previous_html += page_count === 1 || page_number === 1 || page_number === 2 ? ' ' + settings.html_prefix + '-navigation-disabled' : '';
			previous_html += '"><b>...</b></span>';
		...
		function refresh_next(page_number)
		{
			...
			var page_count2 = (page_count - 1);
			var next_page = page_number + 1 > page_count ? page_count : page_number + 1,
			next_html = '<span class="';
			next_html += page_count === 1 || page_number === page_count2 || page_number === page_count  ? ' ' + settings.html_prefix + '-navigation-disabled' : '';
			next_html += '"><b>...</b></span>';
		...
		function refresh_simple_pagination(page_number)
		{
			...
			{
				var page_x_of_x_html = '' + settings.page_x_of_x_content + ' ' + simple_number_formatter(page_number, 0, settings.thousands_separator) + '' + simple_number_formatter(page_count, 0, settings.thousands_separator);
			...
			{
				var showing_x_of_x_html = simple_number_formatter(item_range_min + 1, 0, settings.thousands_separator) + '-' + simple_number_formatter(item_range_max, 0, settings.thousands_separator) + ' ' + settings.showing_x_of_x_content + ' ' + simple_number_formatter(item_count, 0, settings.thousands_separator);
